...
function openConnection(){
    sql.connect('mssql://OpenBankingAdmin:Password@123@open-banking-server.database.windows.net/DemoBankDB')
}
...
router.get('/:id', async (request, response) => {
    ...
    try {
        const result = await sql.query`SELECT * FROM [Parties] WHERE PartyNumber = ${partyNumber}`;
